    ...
    def load_dataSet(self,
        ...
        if dataSet in [
            ...
            if split_mode == "label":
                ...
                graphs = louvainSplitter(data)
                setattr(self, dataSet + '_test_data', graphs[0])
                setattr(self, dataSet + '_data', graphs[1:])
                setattr(self, dataSet + '_total_data', data)
                setattr(self, dataSet + '_test_index', graphs[0].index_orig)

                for i in range(len(graphs)):
                    print("total node num", graphs[i].x.shape[0])
                    print("total edges", graphs[i].edge_index.shape[1] // 2)

                index_list = []
                for i in range(1, len(graphs)):
                    index_list.append(graphs[i].index_orig)
                setattr(self, dataSet + '_index_list', index_list)
        
        elif dataSet in [
                "FedDBLP",
        ]:
            from dblp_new import DBLPNew
            datasets = DBLPNew(self.config['file_path.workdir'], FL=2)
            global_dataset = DBLPNew(self.config['file_path.workdir'], FL=0)[0]

            setattr(self, dataSet + '_num_classes', len(set(torch.flatten(global_dataset.y).numpy())))
            setattr(self, dataSet + '_in_feats', global_dataset.x.shape[1])

            setattr(self, dataSet + '_test_data', datasets[0])
            setattr(self, dataSet + '_data', datasets[1:])
            setattr(self, dataSet + '_total_data', global_dataset)
            setattr(self, dataSet + '_test_index', datasets[0].index_orig)
            index_list = []
            for i in range(1, len(datasets)):
                index_list.append(datasets[i].index_orig)
            setattr(self, dataSet + '_index_list', index_list)
